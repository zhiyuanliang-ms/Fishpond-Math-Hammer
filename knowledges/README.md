...
- **Pure logic stays in `src/lib/dice/`** — no React, no DOM, no CSS. Everything
  there must be unit-testable in isolation. Attack Simulator persistence and
  share-link helpers live beside `dice/` because they intentionally touch
  browser APIs.
...
- **Page-level components in `src/components/`** are thin glue: they call
  ...
  (e.g. `components/attackSimV2/` for the Attack Simulator's cards).
